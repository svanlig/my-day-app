...
function total(c){return Math.max(1,Math.round((D(c.end)-D(c.start))/86400000)+1)}function elapsed(c){let k=todayKey();if(k<c.start)return 0;if(k>c.end)return total(c);return Math.round((D(k)-D(c.start))/86400000)+1}
function count(c){return Object.entries(data.days).filter(([k,x])=>k>=c.start&&k<=c.end&&x.challenges?.[c.id]?.done).length}
function challengeToday(c){let s=cs(c.id);return '<div class="challenge"><div class="challengeTitle">'+esc(c.name)+'</div><div class="challengeDates">'+short(c.start)+' — '+short(c.end)+' · Day '+elapsed(c)+' of '+total(c)+'('+Math.round(elapsed(c)/total(c)*100)+'% duration)</div>'+(c.target?'<div class="details">Daily target: '+esc(c.target)+'</div>':'')+'<div class="bar"><div class="fill" style="width:'+Math.round(elapsed(c)/total(c)*100)+'%"></div></div><div class="meta">'+(s.done?'✓ Today completed':'Today not completed')+'</div><div class="challengeBtns">'+(todayKey()>=c.start&&todayKey()<=c.end?'<button class="primary" onclick="checkin(\''+c.id+'\')">Today\'s check-in</button>':'')+'<button onclick="editChallenge(\''+c.id+'\')">Edit</button>'+(todayKey()>=c.end?'<button onclick="finish(\''+c.id+'\')">Finish & record result</button>':'')+'</div></div>'}
...
function checkin(id){let c=data.challenges.find(x=>x.id===id);if(!c)return;if(todayKey()<c.start||todayKey()>c.end)return;let s=cs(id);$('checkId').value=id;$('checkTitle').textContent=c.name;$('checkComment').value=s.comment||'';$('checkDone').checked=s.done;$('checkDlg').showModal()}
